...
"""
🛡️ HYDRO-CORE MODEL ENGINE (v4.5)
---------------------------------
...
def run_training_pipeline(db_path='campus_water.db', model_output='models/water_model.pkl'):
    ...
    conn.close()
    print(f"✅ Extracted {len(df)} records from SQL.")
    ...
    print(f"🧹 Scrubbed {len(df) - len(df_clean)} outliers (>8000L).")
    ...
    print(f"📊 Training Complete. Accuracy (R2): {score:.4f} | Error: {mae:.2f}L")
    ...
    print(f"💾 Production Model Serialized to: {model_output}")
